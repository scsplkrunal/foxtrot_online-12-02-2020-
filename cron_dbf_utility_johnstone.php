...
$output = '';
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL,'https://foxtrotonlineportal.com/foxtrot_online/cron_extract_db_zip.php?company_name=johnstone&company_abbr=JBS');
$output = curl_exec($ch);
curl_close($ch);  

if($output != '')
{
    $company_array  = array('johnstone'); //array('lifemark','allegheny','concorde','cue','demo','dominion','liberty','signalsecurities','vicuscapital','everspire','westgroup','lafferty');
    $company_abbr   = array('JBS'); //array('Lm','ALG','CON','CUE','DEMO','DOM','LIB','SSR','VC','ES','WSG','LFT');
