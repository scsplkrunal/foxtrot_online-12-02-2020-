...
$output = '';
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL,'https://foxtrotonlineportal.com/foxtrot_online/cron_extract_db_zip.php?company_name=silveroak&company_abbr=SO');
$output = curl_exec($ch);
curl_close($ch);  

if($output != '')
{
    $company_array  = array('silveroak'); //array('lifemark','allegheny','concorde','cue','demo','dominion','liberty','signalsecurities','vicuscapital','everspire','westgroup','lafferty');
    $company_abbr   = array('SO'); //array('Lm','ALG','CON','CUE','DEMO','DOM','LIB','SSR','VC','ES','WSG','LFT');
    $table          = array(
        // 'test',
        '1099FTO',
        'PermrepFTO',
